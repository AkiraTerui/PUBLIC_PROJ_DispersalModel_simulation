# General setup ----
  ...
  PHI <- c(0.4, 0.8)
  PARA <- as.matrix(expand.grid(N, LEN, SIGMA, PHI))
  Kernel <- "Gaussian"
  colnames(PARA) <- c("N", "LEN", "SIGMA", "PHI")
...
# Bayesian Inference ----  
  ...
  for(i in 1:Npara){
    ...
    for(j in 1:Nrep){
      ...
      m <- read.jagsfile("bayes-model/gaussian/model_truncated_gaussian_v1.R")
...
# Save results ----
  colnames(output) <- c("N", "LEN", "SIGMA", "PHI",
                        "Pcap", "Pstay",
                        "sigma_lower", "sigma_med", "sigma_upper",
                        "R_hat_sigma",
                        "MCMCiter", "Burn_in", "N_thin", "N_sample")
  filename <- paste0("result/sim_model_truncated_", Kernel, Sys.Date(), ".csv")
