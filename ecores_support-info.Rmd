...
# Read data ----
  ...
  ## Create columns Bias and CItrue
  dat_Laplace <- dat_Laplace %>%
                    mutate(Bias = delta_med - DELTA) %>%
                    mutate(theta = DELTA) %>%
                    mutate(CItrue = ifelse(DELTA <= delta_upper & DELTA >= delta_lower, 1, 0))
                    
  dat_Gaussian <- dat_Gaussian %>%
                    mutate(Bias = sigma_med - SIGMA) %>%
                    mutate(theta = SIGMA) %>%
                    mutate(CItrue = ifelse(SIGMA <= sigma_upper & SIGMA >= sigma_lower, 1, 0))

# Table ----
  ## define function
  fun_table <- function(data, parameter = c("N", "LEN", "DELTA", "SIGMA", "PHI")){
    paraID <- which(colnames(data) %in% parameter)
    para <- sapply(paraID, function(i)unique(data[,i]) )
    para.set <<- expand.grid(N = para[[1]],
                            LEN = para[[2]],
                            THETA = para[[3]],
                            PHI = para[[4]])
    
    for(i in 1:nrow(para.set)){
      Ind <- sapply(1:length(paraID), function(x) data[,paraID[x]]==para.set[i,x])
      dat.sub <<- data[rowSums(Ind) == length(paraID),]
      uvalue <<- sapply(dat.sub[,paraID], unique)
      if(any(uvalue != para.set[i,1:length(paraID)]) ) { print(dat.sub); stop("error") }
      if(length(unique(dat.sub$theta)) != 1) { print(dat.sub); stop("error") }
      para.set[i,c("m1_Bias", "m2_Bias", "m3_Bias")] <- tapply(dat.sub$Bias, dat.sub$model_code,
                                                               function(x) round(100*mean(x)/unique(dat.sub$theta), 1) )
      para.set[i,c("m1_CIp", "m2_CIp", "m3_CIp")] <- tapply(dat.sub$CItrue, dat.sub$model_code, mean)
    }
  ...
  ## Split dataframes
  reL <- fun_table(dat_Laplace); reL$Kernel <- "Laplace"
  reG <- fun_table(dat_Gaussian); reG$Kernel <- "Gaussian"
  re <- merge(reL, reG, all = T)
  
  theta <- seq(50, 300, by = 50)
  DL <- list(NULL)
  for(i in 1:length(theta)){
    DL[[i]] <- re %>%
      dplyr::filter(THETA == theta[i]) %>%
      dplyr::select(Kernel, N, LEN, PHI, m1_Bias, m2_Bias, m3_Bias) %>%
      dplyr::arrange(Kernel) %>%
      kable(col.names = c("Kernel", "$N$", "$L$", "$\\phi$",
...
# Read data ----
  ...
  ytitle <- expression("95% CI of estimated dispersal parameter"~delta[est])
...
## axis titles
  ...
  ytitle <- expression("95% CI of estimated dispersal parameter"~sigma[est])
