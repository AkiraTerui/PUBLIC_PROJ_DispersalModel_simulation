...
# Plot ----
  emf("figure_delta_v1.emf", width = 11, height = 13.5)
  #pdf("figure_delta_v1.pdf", width = 11, height = 13.5)
  ...
  for(i in 1:nrow(para.set)){
    ...
    ## Plot
    plot(delta_med ~ delta, dat.sub, pch = 21, cex = 1.2, xlim = c(0,300), ylim = c(0,max(dat.sub$delta_med)),
    ...
    ## Polygon - underestimated area
    polygon(x = c(0, 300, 300, 0), y = c(0, 0, 300, 0), col = grey(0, 0.1), border = NA)
    
    ## Lowess fit
    fit1 <- lowess(dat.sub$delta_med[dat.sub$model=="disp_obs"] ~ dat.sub$delta[dat.sub$model=="disp_obs"])
    fit2 <- lowess(dat.sub$delta_med[dat.sub$model=="truncated"] ~ dat.sub$delta[dat.sub$model=="truncated"])
    fit3 <- lowess(dat.sub$delta_med[dat.sub$model=="simple"] ~ dat.sub$delta[dat.sub$model=="simple"])
  ...
  mtext(expression("Estimated dispersal distance"~~delta[est]~"(m)"), 2, outer = T, line = 1, cex = 1.3)
